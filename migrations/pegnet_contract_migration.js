const PegnetContracts = artifacts.require('ChainlinkExample')
  ...
  if (!network.startsWith('live')) {
    ...
    deployer.deploy(LinkToken, { from: defaultAccount }).then(link => {
      return deployer
        ...
        .then(() => {
          return deployer.deploy(PegnetContracts, link.address)
        })
    })
  } else {
    // For live networks, use the 0 address to allow the ChainlinkRegistry
    // contract automatically retrieve the correct address for you
    deployer.deploy(PegnetContracts, '0x0000000000000000000000000000000000000000')
  }
}
